...
while($res=mysql_fetch_array($sql,MYSQL_NUM))
{
	...
	if($chkpresent==0)
	{
		mysql_query("insert into sessional(rollno,scode,revision_year) values('$res[2]','$scode','$revision_year')",$conn) or die("Error2: ".mysql_error());
	}
	$marks=mysql_result(mysql_query("select s1 from sessional where rollno='$res[2]' and scode='$scode' and revision_year='$revision_year'",$conn),0);
